...
class SensorsInterface(QMainWindow):  # Define a class that inherits from QMainWindow
    ...
    def start_data(self):
        ...
        libmetawear.mbl_mw_gyro_bmi270_set_range(device.board, GyroBoschRange._1000dps)
        libmetawear.mbl_mw_gyro_bmi270_set_odr(device.board, GyroBoschOdr._50Hz)
        libmetawear.mbl_mw_gyro_bmi270_write_config(device.board)

        gyro = libmetawear.mbl_mw_gyro_bmi270_get_packed_rotation_data_signal(device.board)
        libmetawear.mbl_mw_datasignal_subscribe(gyro, None, stateInstance.callback)
